...
class EditorActivity : AppCompatActivity() {
    ...
    private val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            // Requerimiento: Pausar música normal al entrar al editor para trabajar limpio
            if (!musicService!!.isEditorMode()) {
    ...
    private fun setupListeners() {
        ...
        binding.effectsLayout.sbVolume.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(s: SeekBar?, p: Int, f: Boolean) { if(f) musicService?.applyVolume(p/100f) }
        ...
        binding.effectsLayout.sbPitch.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(s: SeekBar?, p: Int, f: Boolean) {
                if(f) {
                    ...
                    val speed = 0.5f + (binding.effectsLayout.sbTempo.progress / 50f)
                    musicService?.applyPlaybackParams(pitch, speed)
                }
            }
            override fun onStartTrackingTouch(s: SeekBar?) {}
            override fun onStopTrackingTouch(s: SeekBar?) {}
    ...
    private fun updateEditorUI() {
        ...
        if (song != null) {
            ...
            // Actualizar visibilidad del círculo en el editor basado en si hay imagen
            if (song.coverUri != null) {
                binding.flEditorDisc.visibility = View.VISIBLE
                binding.vEditorDiscBorder.visibility = View.VISIBLE
                Glide.with(this).load(song.coverUri).centerCrop().into(binding.ivEditorArt)
                
                binding.flSmallDisc.visibility = View.VISIBLE
                binding.vSmallDiscBorder.visibility = View.VISIBLE
                Glide.with(this).load(song.coverUri).centerCrop().into(binding.ivSmallArt)
            } else {
                binding.flEditorDisc.visibility = View.GONE
                binding.vEditorDiscBorder.visibility = View.GONE
                binding.flSmallDisc.visibility = View.GONE
                binding.vSmallDiscBorder.visibility = View.GONE
            }
        }
    }
